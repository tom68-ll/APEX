...
import psutil #for mem management
...
class BasicTrainer(object):
    ...
    def initialize_adapter_pool(self, task_id, adapter_skeleton) -> dict:
        ...
        key_num = self.args.initialize_pool_size * self.args.task_num
        # Initialize random keys
        num_features = self.encoder_model.config.hidden_size
        random_keys = torch.randn(key_num, num_features, device=self.device)

        for idx, random_key in enumerate(random_keys):
            task_embedding_param = nn.Parameter(random_key.unsqueeze(0))
    ...
    def load_adapter(self,adapter_name : str) -> None:
        #where its right ATTENTION!
        self.model.load_adapter(os.path.join(self.root_adapter_path,adapter_name))
    ...
    def make_train_classification(self,examples) -> dict:
        ...
        for i in range(0, len(texts_to_embedding), batch_size):
            ...
            query_list.extend(torch.unbind(embeddings, dim=0))
        for query in query_list:
            query = query.unsqueeze(0)
        assert len(query_list) == len(examples)
        #now make all the key eval
        for key in self.task_keys:
            key.requires_grad = False
        #now classification
        examples_classified = {}
        for exam_idx, example in enumerate(examples):
            max_score = -1
            class_of_this = -1
            for key_idx in range(len(self.task_keys)):
                key = self.task_keys[key_idx]
                score = self.get_scores(query_list[exam_idx].unsqueeze(0),key)
                if score > max_score:
                    max_score = score
                    class_of_this = key_idx
            if class_of_this not in list(examples_classified.keys()):
                examples_classified[class_of_this] = []
            examples_classified[class_of_this].append(example)

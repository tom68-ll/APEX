...
sys.path.append("..")
import random
from amtm_test.pp_trainer import BasicTrainer
import os
import time
...
class AdapterPoolingTrainer(BasicTrainer):
    def __init__(self, args, model_save_path):
        super(AdapterPoolingTrainer, self).__init__(args, model_save_path)
    ...
    def train(self):
        ...
        if self.args.continue_time:
            self.root_adapter_path = os.path.join(self.root_adapter_path,self.args.continue_time)
        else:
        ...
        for task_id in range(self.args.continue_task_id,self.args.task_num):
            ...
            if task_id == self.args.continue_task_id and self.args.model_load_path == "":
                ...
                #self.eval_task_stream(task_id,adapter_skeleton)
            ...
            print("initialize ok")
            examples_dict = self.make_train_classification(examples)
            dev_examples_dict = self.make_train_classification(dev_examples)

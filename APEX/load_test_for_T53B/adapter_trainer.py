...
class BasicTrainer(object):
    ...
    def __init__(self, args, model_save_path):
        ...
        #optimizer_grouped_parameters = self.make_optimizer_groups()
    ...
    #choose a model's encoder as the query function
    '''
    def initialize_adapter_pool(self, task_id, adapter_skeleton) -> dict:
        self.encoder_model.eval()
        self.task_keys = []  # Dictionary to store the averaged embeddings for each task
        #initialize every task's key
        if task_id == 0:
            key_num = 10
        else:
            key_num = 5
        data_input_samples = random.sample(self.task_controller.task_list[task_id]["train"],10 * key_num)
        for key_id in range(key_num):
            #sample 10 embedding, avg pooling and get the mean of them as the key initialization
            sample_embeddings = []
            for sample in data_input_samples[key_id*10 : key_id*10 + 10]:
                ...
                sample_embeddings.append(encoder_outputs.last_hidden_state.mean(dim=1))
            # Compute the mean across the 10 samples for the current task
            task_embedding = torch.stack(sample_embeddings).mean(dim=0)
            task_embedding_param = nn.Parameter(task_embedding)
            self.task_keys.append(task_embedding_param)

            #now give the adapter and create directory
    ...
    def initialize_adapter_pool(self, task_id, adapter_skeleton) -> dict:
        ...
        if self.args.initialize_pool_method == "special_kmeans":
            ...
            # Perform K-Means Clustering, assuming all_embeddings_tensor is 2D [num_samples, num_features]
    ...
    def save_key(self,task_key) -> None:
        print(os.path.join(self.adapter_path,"key.pt"))
        torch.save(task_key,os.path.join(self.adapter_path,"key.pt"))

    def load_key(self,key_path):
        return torch.load(os.path.join(key_path,"key.pt"), map_location=self.device)

    def load_all_keys(self,root_adapter_path,adapter_skeleton,total_num):
        for task_id in range(total_num):
            save_path = os.path.join(root_adapter_path,adapter_skeleton + str(task_id))
            if not os.path.exists(os.path.join(save_path,"key.pt")):
                self.task_keys.append(torch.zeros(self.task_keys[0].shape).to(self.device))
            else:
                self.task_keys.append(self.load_key(save_path))
    ...
    def train_one_epoch_with_adapter(self, examples, adapter_name, key_idx, optimize_step=True):
        ...
        # make a new optimizer for adapter
        #adapter_parameters = [
        #        {'params': self.model.parameters(), 'lr': self.args.lr}
        #    ]
        ...
        #adapter_scheduler = StepLR(adapter_optimizer, step_size=1, gamma=0.1)
    ...
    def epoch_acc(self, examples, name_skeleton, batch_size=4, cur_task=True, save_prediction = False,task_now = 0,test_on = 0):
        self.model.eval()
        correct_predictions = 0
        ...
        pbar = tqdm(total = choose_all)
        score_to_store = []
        if self.args.load_test_trigger == "all_load":
            for example_id,example in enumerate(examples):
                ...
                score_list = []
                task_keys_range = ( task_now + 1 ) * self.args.initialize_pool_size
                score_list = []

                for idx in range(task_keys_range):
        ...
        if self.args.load_test_trigger == "all_load_mix":
            for example_id,example in enumerate(examples):
                pbar.update(1)
                text, sql = self.prepare_data(example)
                information = example
                ...
                score_maxes = heapq.nlargest(self.args.top_n, score_list)
                index_maxes = list(map(score_list.index, score_maxes))

                for index in index_maxes:
                    self.load_adapter(name_skeleton + str(index))

                exp_scores = [math.exp(score) for score in score_maxes]
                total_exp = sum(exp_scores)
                weights = [exp_score / total_exp for exp_score in exp_scores]

                adapter_names = [name_skeleton + str(index) for index in index_maxes]
                self.model.average_adapter("avg", adapter_names, weights=weights)

                self.model.set_active_adapters("avg")
                self.model.to(self.device)

                for index in index_maxes:
                    self.delete_adapter(name_skeleton + str(index))
                ...
                for i, prediction in enumerate(predictions):
                    ...
                    if "".join(prediction.strip().split()) == "".join(sqls[i].strip().split()):
                        correct_predictions += 1
                        right_result.append((example, sql, prediction))
                    else:
                        wrong_result.append((example, sql, prediction))

                self.delete_adapter("avg")

        if self.args.load_test_trigger == "choose_load_mix":
            #scores_to_s = []
            batch_data = defaultdict(list)
            for example_id, example in enumerate(examples):
                pbar.update(1)
                text, sql = self.prepare_data(example)
                db_id_now = example['db_id']

                query = self.get_queries(text)
                query_embeddings = self.project_layer
                ...
                for idx in range(task_keys_range):
                    if torch.equal(self.task_keys[idx], torch.zeros(self.task_keys[0].shape).to(self.device)):
                        score_list.append(-1)
                    else:
                        score = self.get_scores(query, self.task_keys[idx], type='cosine')
                        score_val = score.cpu().detach().numpy().item()
                        score_list.append(score_val)
                        valid_scores.append(score_val)

                def calculate_z_scores(similarities, invalid_value=-1):
                    valid_similarities = [s for s in similarities if s != invalid_value]
                    if len(valid_similarities) == 0:
                        return [-float('inf')] * len(similarities)

                    mean_similarity = np.mean(valid_similarities)
                    std_similarity = np.std(valid_similarities)

                    z_scores = (np.array(similarities) - mean_similarity) / std_similarity

                    z_scores[similarities == invalid_value] = -float('inf')

                    return z_scores

                z_Scores = calculate_z_scores(np.array(score_list))
                z_Score = np.max(z_Scores)

                if z_Score > self.args.seperate_number:
                    chosen_key = score_list.index(max(score_list))

                    if torch.equal(self.task_keys[chosen_key], torch.zeros(self.task_keys[0].shape).to(self.device)):
                        raise ValueError("choose the zero key!!!")

                    batch_data[chosen_key].append((text, sql, db_id_now, example_id))
                else:
                    score_maxes = heapq.nlargest(self.args.top_n, score_list)
                    index_maxes = list(map(score_list.index, score_maxes))
            ...
            for chosen_key, examples_for_key in batch_data.items():
                self.load_adapter(name_skeleton + str(chosen_key))

                texts = [ex[0] for ex in examples_for_key]
                sqls = [ex[1] for ex in examples_for_key]
                db_ids = [ex[2] for ex in examples_for_key]
                example_ids = [ex[3] for ex in examples_for_key]

                for batch_start in range(0, len(texts), batch_size):
                    batch_texts = texts[batch_start:batch_start + batch_size]
                    batch_sqls = sqls[batch_start:batch_start + batch_size]
                    batch_db_ids = db_ids[batch_start:batch_start + batch_size]
                    batch_example_ids = example_ids[batch_start:batch_start + batch_size]

                    input_encodings = self.tokenizer(list(batch_texts), padding=True, truncation=True, return_tensors="pt")
                    ...
                    with torch.no_grad():
                        outputs = self.model.generate(
                            ...
                            max_length=self.args.max_generate_length,
                            **self.generation_arguments
                        )
                    ...
                    for i, prediction in enumerate(predictions):
                        all_result[batch_example_ids[i]] = {
                            "db_id": batch_db_ids[i],
                            "example": batch_texts[i],
                            "sql": batch_sqls[i],
                            "prediction": prediction
                        }
                        if "".join(prediction.strip().split()) == "".join(batch_sqls[i].strip().split()):
                            correct_predictions += 1
                            right_result.append((examples[batch_example_ids[i]], batch_sqls[i], prediction))
                        else:
                            wrong_result.append((examples[batch_example_ids[i]], batch_sqls[i], prediction))

                self.delete_adapter(name_skeleton + str(chosen_key))
                torch.cuda.empty_cache


        if self.args.load_test_trigger == "choose_load_mix_two":
            for example_id,example in enumerate(examples):
                pbar.update(1)
                text, sql = self.prepare_data(example)
                information = example
                #db_id_now = information['example']['db_id']
                db_id_now = information['db_id']
                ...
                for idx in range(task_keys_range):
                    self.task_keys[idx].requires_grad = False
                    score = self.get_scores(query, self.task_keys[idx], type='cosine')
                    score_list.append(score.cpu().detach().numpy())

                def calculate_z_scores(similarities):
                    mean_similarity = np.mean(similarities)
                    std_similarity = np.std(similarities)
                    z_scores = (similarities - mean_similarity) / std_similarity
                    return np.max(z_scores)
                z_Score = calculate_z_scores(np.array(score_list))
                ...
                if z_Score > self.args.seperate_number:
                    ...
                    def calculate_outerindex(score_list):
                        avg_scores = []
                        for idx in range((int)(len(score_list) / self.args.initialize_pool_size)):
                            avg_scores.append(sum(score_list[self.args.initialize_pool_size * idx : self.args.initialize_pool_size * (idx + 1)]) / self.args.initialize_pool_size)
                        return avg_scores
                    avg_scores = calculate_outerindex(score_list)
                    if len(avg_scores) > self.args.top_n:
                        score_maxes = heapq.nlargest(self.args.top_n, avg_scores)
                    else:
                        score_maxes = avg_scores
                    index_maxes = list(map(avg_scores.index, score_maxes))
                    name_skeleton1 = name_skeleton
                    for index in index_maxes:
                        tmp_rootpath = self.root_adapter_path
                        self.root_adapter_path = "model_saved/adapterpool/maybe11_combine/20240807-1505"
                        self.load_adapter(name_skeleton1 + str(index))
                        self.root_adapter_path = tmp_rootpath

                    exp_scores = [math.exp(score) for score in score_maxes]
                    total_exp = sum(exp_scores)
                    weights = [exp_score / total_exp for exp_score in exp_scores]

                    adapter_names = [name_skeleton1 + str(index) for index in index_maxes]
                    self.model.average_adapter("avg", adapter_names, weights=weights)
                    ...
                    for index in index_maxes:
                        self.delete_adapter(name_skeleton1 + str(index))
        ...
        if self.args.load_test_trigger == "have_a_try":
            name_skeleton = "adapter_key"
            all_score_list = []
            help_tmp = True
            for example_id, example in enumerate(examples):
                pbar.update(1)
                ...
                task_keys_range = (task_now + 1) * self.args.initialize_pool_size
                score_list = []
                for index in range(task_keys_range):
                    self.load_adapter(name_skeleton + str(index))
                    self.model.train()
                    self.model.zero_grad()  # Clear previous gradients
                    _, _, model_loss = self.train_one_batch([example], 0, 0)
                    model_loss.backward()  # Backpropagate to compute gradients
                    
                    # Extract gradients from the model
                    gradients = [param.grad.view(-1) for param in self.model.parameters() if param.grad is not None]
                    if help_tmp:
                    ...
                    noise_power = torch.var(gradients)
                    gsnr = signal_power / noise_power

                    # **Convert tensor to float before appending**
                    score_list.append(gsnr.item())

                    self.delete_adapter(name_skeleton + str(index))
                all_score_list.append(score_list)

            store_path = os.path.join(
                "result/20240825/combine_grad_score_check",
                f"task_{str(task_now)}_test_on_{str(test_on)}.json"
            )

            with open(store_path, 'w') as fp:
                json.dump(all_score_list, fp)

        if self.args.load_test_trigger == "ideal_all_load":
            for example_id,example in enumerate(examples):
                pbar.update(1)
                text, sql = self.prepare_data(example)
                information = example
                ...
                for idx in range(task_keys_range - 1 * self.args.initialize_pool_size , task_keys_range):
                    self.task_keys[idx].requires_grad = False
                    score = self.get_scores(query, self.task_keys[idx], type='cosine')
        ...
        if self.args.load_test_trigger == "ideal_all_load_ver2":
            self.load_adapter(name_skeleton + str(task_now))
            for batch_start in tqdm(range(0, len(examples), batch_size)):
                ...
                texts, sqls = zip(*[self.prepare_data(example) for example in current_batch])
                db_ids = [example['db_id'] for example in current_batch]
                example_ids = list(range(batch_start,batch_end))
            ...
            self.delete_adapter(name_skeleton + str(task_now))
        
        if self.args.load_test_trigger == "get_zscores":
            scores_to_s = []
            for example_id, example in enumerate(examples):
                pbar.update(1)
                text, sql = self.prepare_data(example)
                ...
                z_Score = calculate_z_scores(np.array(score_list))
                scores_to_s.append(z_Score.tolist())


            
            with open(os.path.join(self.args.prediction_save_path,f"task_{task_now}_teston_{test_on}.json"),'w') as fp:
                json.dump(scores_to_s,fp)
            return 0,0,(right_result, wrong_result, None), None
            
        if self.args.load_test_trigger == "test_T5":
        ...
        #BUG here you should cancel the annotation
        results_output = []
        ...
        '''
                    ...
                    print(score)
                    score_list.extend(score.cpu().detach().numpy().tolist()[0])
                score_to_store.append(score_list)
        with open(os.path.join("result/20240825/combine_seperatenumber_contrast",f"task_{task_now}_teston_{test_on}.json"),'w') as fp:
            json.dump(score_to_store,fp)
            
        return 0, chosen_right_rate, (right_result, wrong_result, None), None
        '''
        '''
    '''

...
class BasicTrainer(object):
    def __init__(self, args, model_save_path):
        ...
        # determine PET type
        if self.args.pet_type == 'adapter':
            self.pet_config = BnConfig(mh_adapter=True, output_adapter=True, reduction_factor=16, non_linearity="relu")
        elif self.args.pet_type == 'lora':
            self.pet_config = LoRAConfig(r=8, alpha=16)
        else:
    ...
    def find_optimal_dbscan_parameters(self, data, n_neighbors=5):
        for n_neighbors in range(1,20):
            ...
            plt.plot(distances)
            plt.savefig(f'picture/{n_neighbors}_distance_plot.png')
        
        return distances

    def initialize_adapter_pool(self, task_id, adapter_skeleton) -> dict:
        #TODO only random realize the adapter config, while others could only utilize adapter instead of lora
        ...
        if self.args.initialize_pool_method == "special_kmeans":
        ...
        elif self.args.initialize_pool_method == "special_dbscan":
            # Determine the optimal parameters for DBSCAN, e.g., eps and min_samples
            self.key_belong = []
            
            distances = self.find_optimal_dbscan_parameters(all_embeddings_tensor.cpu().numpy())
            eps_value = 0.1
            min_samples_value = 5

            dbscan = DBSCAN(eps=eps_value, min_samples=min_samples_value)
            labels = dbscan.fit_predict(all_embeddings_tensor.cpu().numpy())
            ...
            for label in unique_labels:
                ...
                cluster_embeddings = all_embeddings_tensor[labels == label]
                cluster_center = cluster_embeddings.mean(dim=0).cpu().numpy()
            ...
            if self.args.initialize_pool_method == "random" or task_id == 0:
                for center in cluster_centers:
                    ...
                    self.load_adapter(adapter_skeleton + "_initiate",load_as = adapter_name)
                    self.save_delete_adapter(adapter_name)
    ...
    def save_key(self,task_key,adapter_name: str) -> None:
    ...
    def get_queries_in_batches(self, texts, batch_size):
        batch_queries = []
        
        for i in range(0, len(texts), batch_size):
            batch_texts = texts[i:i+batch_size]
            batch_query = self.get_queries(batch_texts)
            batch_queries.append(batch_query)
        
        batch_queries = torch.cat(batch_queries, dim=0)
        
        return batch_queries

    def get_key_loss_cl(self, examples, current_key, other_keys, total_scores, temperature = 0.07):
        if self.args.key_initialize_method == "question":
            texts = [example['text'].split('|')[1] for example in examples] 
        elif self.args.key_initialize_method == "text":
            texts = [example['text'] for example in examples] 
        ...
        batch_size = 8
        batch_queries = self.get_queries_in_batches(texts, batch_size)

        batch_queries_projected = self.project_layer(batch_queries)
        projected_key_pos = self.project_layer(current_key)
        projected_keys_neg = torch.stack([self.project_layer(other_keys[i]) for i in range(len(other_keys))], dim=0)

        positive_similarity = F.cosine_similarity(batch_queries_projected, projected_key_pos, dim=-1)
        negative_similarities = [F.cosine_similarity(batch_queries_projected, projected_keys_neg[i], dim=-1) for i in range(projected_keys_neg.size(0))]
        negative_similarities = torch.stack(negative_similarities, dim=1)
        all_similarities = torch.cat([positive_similarity.unsqueeze(1), negative_similarities], dim=1)

        all_similarities = all_similarities / temperature
        labels = torch.zeros(all_similarities.size(0), dtype=torch.long).to(self.device)
        cl_loss = F.cross_entropy(all_similarities, labels)

        scores = self.get_scores(batch_queries, current_key)
        prompt_loss = -1 * self.args.pool_lambda * scores.sum()
        score_sum = scores.detach().sum().item()
        total_scores += score_sum

        return  total_scores , prompt_loss, cl_loss 

    def train_one_epoch_with_adapter_sft(self, examples, adapter_name, key_idx, optimize_step=True):
    ...
    def train_one_epoch_with_adapter(self, examples, adapter_name, key_idx, optimize_step=True):
        ...
        adapter_parameters.append({'params' : [p for n, p in self.project_layer.named_parameters() if p.requires_grad], 'lr': 1e-5})
        ...
        while st < len(examples):
            ed = st + self.args.batch_size if st + self.args.batch_size < len(examples) else len(examples)
            report_loss, example_num, model_loss = self.train_one_batch(examples[st:ed], report_loss, example_num)
            tmp_task_keys = copy.deepcopy(self.task_keys)
            tmp_task_keys.pop(key_idx)
            total_scores,key_loss,cl_loss = self.get_key_loss_cl(examples[st:ed],self.task_keys[key_idx],tmp_task_keys,total_scores)
                       
            loss = key_loss + model_loss + 0.1 * cl_loss
    ...
    def train_one_epoch(self, examples, adapter_name, optimize_step=True):
        # Freeze T5 parameters
        for param in self.model.base_model.parameters():
            param.requires_grad = False
        ...
        adapter_optimizer = torch.optim.Adam(adapter_parameters, lr=self.args.lr)
        random.shuffle(examples)
        report_loss = 0.0
        total_scores = 0.0
        st = 0
    ...
    def batch_epoch_acc(self, examples, batch_size=16, save_prediction=False, task_now=0, test_on=0):
        self.model.eval()
    ...
    def epoch_acc(self, examples, name_skeleton, batch_size=16, cur_task=True, save_prediction = False,task_now = 0,test_on = 0):
        self.model.eval()
        correct_predictions = 0
